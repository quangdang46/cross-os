...
// TestLockExclusiveRefusesSecondHolder pins the property every per-GOOS
// lock file has to keep, whichever syscall backs it: a second opener of the
// same lock file must be told errLockHeld. A stub returning nil fails OPEN —
// the second daemon reads the lock as free, unlinks the live daemon's socket
// and serves beside it — so this test is the guard against that, on whatever
// GOOS runs it. Platforms with no advisory locking (lock_other.go) skip: they
// refuse to start rather than serve unlocked, which is a different contract.
func TestLockExclusiveRefusesSecondHolder(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("open first holder: %v", err)
	}
	defer first.Close()
	if err := lockExclusive(first); err != nil {
		...
		t.Fatalf("first holder must get the lock: %v", err)
	}

	second, err := os.OpenFile(lockPath, os.O_CREATE|os.O_RDWR, 0o600)
	if err != nil {
		t.Fatalf("open second holder: %v", err)
	}
	defer second.Close()
	if err := lockExclusive(second); !errors.Is(err, errLockHeld) {
		t.Fatalf("second holder: err=%v, want errLockHeld — a lock that fails open lets a second daemon steal the socket", err)
	}
}
