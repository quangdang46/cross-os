...
Source repository: mariusgm/newfile
Source commit: b3f665ab839dfe95e6925735ca2a8209d3100fb8
Source file: App/PreferencesView.swift:20-34 (the debounced persist and the validOnly filter at its sink)
Original license: MIT
Original copyright: Copyright (c) 2026 Wheel Up Labs
CrossOS destination: app/frontend/src/controls/FileTypeListControl.tsx (scheduleField, and the re-arm in saveField)
Modification: structural port, no code copied. The RULE transfers, and the reference records its own reason for it: "Persist off a debounce, not per keystroke: every edit used to JSON-encode the full list into UserDefaults synchronously from the row's onChange." Two keystroke-driven writes to one row collapse into one, and the write that lands carries the LAST keystroke rather than racing four versions of the same word. Two differences, both forced by the verb rather than chosen. The reference debounces the WHOLE list because it persists the whole list; here SET_FILE_TYPE is a validated read-modify-write of ONE existing row, so the debounce is per row. And the reference's sink applies validOnly on the way to the store — the other half of its rule — which here is enforced by extensionProblem refusing the write rather than by filtering the table, because the daemon is the boundary that decides what may exist and a shell-side filter would be a second opinion. The re-arm when a write is already in flight has no reference: the debounce makes the window reachable, and dropping the edit there would lose the word a person was in the middle of typing.
Reason for modification: language and medium port — a Combine sink over RunLoop.main becomes a timer in a React renderer, and the two must be cleared on unmount rather than torn down with the view. §9.11 records newfile as DIRECT REUSE; this entry adds no copied code and no new license obligation.
CrossOS license: MIT
